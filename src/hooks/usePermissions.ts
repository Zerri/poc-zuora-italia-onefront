...
export const usePermissions = () => {
  ...
  return {
    ...
    canCreate: (resource: string) => user?.permissions[resource]?.CREATE || false,
    canDelete: (resource: string) => user?.permissions[resource]?.DELETE || false,
    canExport: (resource: string) => user?.permissions[resource]?.EXPORT || false,
    hasRole: (role: string) => user?.roles?.includes(role) || false,
    isAdmin: () => user?.roles?.includes('ADMIN') || false,
    
    // Getter per dati utente comuni
    get userName() { return user?.name || 'Guest'; },
    get userId() { return user?.id || ''; },
    get userRoles() { return user?.roles || []; },
